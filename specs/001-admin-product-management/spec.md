...
- **FR-016**: Owner MUST be able to mark any processing option as not yet available for a customer to select (e.g., the "bring your own design"/"custom design service" options from FR-004, which have no working customer-facing order flow yet), so a later customer-facing feature can safely exclude it rather than infer this from its label text.
- **FR-017**: Owner MUST be able to record a product's shipping weight and package dimensions (length, width, height), optionally, so a later checkout feature can calculate real carrier shipping rates rather than guessing.
...
- FR-016's "not yet customer-selectable" marker defaults to off for a new processing option; the owner only turns it on for options like "bring your own design"/"custom design service" where no working customer order flow exists yet.
- FR-017's weight/dimensions represent one reasonable packaged value for the product as typically shipped, not a value that varies per selected option (e.g., per size) — accurate enough for a small business's carrier-rate needs; revisit only if this proves materially wrong for specific products.
